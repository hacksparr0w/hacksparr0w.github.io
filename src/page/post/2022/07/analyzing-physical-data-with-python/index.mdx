...
Python code is just plain text, so you can use any text editor of your liking
to write Python programs. Text editors come with varying degrees of support
for programming languages. There are lightweight but still very powerful text
editors like [Sublime Text][4] that offer syntax-highlighting, autocompletion,
and some useful graphical features, but usually don't go beyond that. Then
there are so-called integrated development environments (IDEs), which come
with full support for your language of choice, providing features like a
built-in debugger, dependency management, test runner, and so on. A very
popular IDE for Python is [PyCharm][5]. My personal favorite is
...
The following is a list of Python packages for scientific computing that we'll
be using in our examples.

 - [NumPy][9] and [pandas][10]: NumPy is a Python library for performing math
 operations on arbitrary N-dimensional arrays. Pandas further extends this
 functionality with concepts such as data grids.

 - [Uncertainties][11], [Pint][12], [Pint-Pandas][13]: The uncertainties
 package makes Python play well with quantities that are subject to
 measurement uncertainty. Pint and Pint-Pandas packages bring physical units
 to the world of Python.

 - [SciPy][14]: SciPy provides algorithms for optimization, differential
 equations, statistics, and many other classes of problems commonly found in
 physical applications.

 - [SymPy][15]: SymPy is a Python package for symbolic mathematics. It lets
 you perform algebraic operations on your math equations directly in your
 Python programs. We will be using this package for working with $\LaTeX$
 math expressions.

 - [Plotly][16] and [Kaleido][17]: Plotly is a modern Python library for data
 visualization. It's a low-code solution for making interactive,
 publication-quality graphs. Kaleido will let us export Plotly graphs into
 static images.

Working with Python dependencies is a complex subject, and I would recommend
you look into tools like [virtualenv][18] and [Poetry][19]. To keep things
simple, you can just stick with Python's good old package installer,
[pip][20], which usually gets installed alongside the Python binaries. To
install all the listed packages at once, use the following shell command.

```sh
pip install numpy pandas uncertainties pint pint-pandas scipy sympy plotly kaleido
```
...
we'll gradually move on to more advanced exercises.
...
standard metric ruler. The proper way to take measurements and evaluate
uncertainties with a standard metric ruler would be the
[Type A evaluation of standard uncertainty][21]. According to this standard,
we need to take a sample of $N$ independent measurements. After that, we can
estimate the true value of the measured variable as the [sample mean][22] and
the standard uncertainty as the [standard deviation of the mean][23]
(sometimes referred to as standard error). Given $N$ observations $x_i$, the
sample mean $\bar x$ can be calculated as
...
real world, we'll simulate measurements of the sides of our
cuboid as sampling a [normally distributed set of values][24]. We'll define
...
normal distributions will be $\sigma = 0.05 \\, \mathrm\{cm\}$, which is
one-half of the smallest measurement unit of the standard metric ruler. To
generate the measurement values with Python, we'll use the function
[`numpy.random.normal`][25]. Open up your Python REPL, import NumPy,
...
distributed set with a mean of $\mu = 1.6$ and a standard deviation of
$\sigma = 0.05$. These values seem pretty good, but a standard metric ruler
does only have a precision of one millimeter, so we'll have to round our
values with the [`numpy.ndarray.round`][26] function.
...
down your measurements in a notebook or use a laptop or a tablet and save the
measured values into a file on your hard drive. We are going to use the
[`pandas.read_csv`][27] function to load our data from disk. This function
requires our data to be in the [CSV format][28]. CSV stands for
...
separated by a comma (spaces or tabs can also be used for separating
...
it into a file named _data.csv_. Now, we'll load the data using
`pandas.read_csv`, extract the measured values into separate variables, and
use the functions [`pandas.Series.mean`][29] and [`scipy.stats.sem`][30] to
compute the previously defined sample mean and standard deviation of the mean
for these variables. Here's the complete solution.
...
and calculate its means and uncertainties. This is a pretty good start, but
there's still a lot of room for improvement. Firstly, if we run our example,
...
Here we've used the
[`pint_pandas.pint_array.PintDataFrameAccessor.quantify`][31] function to
make our data frame aware of our physical units. We've then used the
[`pint.facets.measurement.MeasurementQuantity.plus_minus`][32] function to
...
[uncertainty propagation theory][33]. Let's try to calculate the volume of our
...
[11]: https://pythonhosted.org/uncertainties/
[12]: https://pint.readthedocs.io/en/stable/
[13]: https://github.com/hgrecco/pint-pandas
[14]: https://scipy.org/
[15]: https://www.sympy.org/en/index.html
[16]: https://plotly.com/python/
[17]: https://github.com/plotly/Kaleido
[18]: https://virtualenv.pypa.io/en/latest/
[19]: https://python-poetry.org/
[20]: https://pypi.org/project/pip/
[21]: https://physics.nist.gov/cuu/Uncertainty/typea.html
[22]: https://en.wikipedia.org/wiki/Sample_mean_and_covariance
[23]: https://en.wikipedia.org/wiki/Standard_deviation#Standard_deviation_of_the_mean
[24]: https://en.wikipedia.org/wiki/Normal_distribution
[25]: https://numpy.org/doc/stable/reference/random/generated/numpy.random.normal.html
[26]: https://numpy.org/doc/stable/reference/generated/numpy.ndarray.round.html
[27]: https://pandas.pydata.org/docs/reference/api/pandas.read_csv.html
[28]: https://en.wikipedia.org/wiki/Comma-separated_values
[29]: https://pandas.pydata.org/docs/reference/api/pandas.Series.mean.html
[30]: https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.sem.html
[31]: https://github.com/hgrecco/pint-pandas/blob/cf527e48557a1e028c6f2d4e628aa7a6cd1b30d4/pint_pandas/pint_array.py#L802
[32]: https://github.com/hgrecco/pint/blob/b0316ce54a90a25845806d7d71fb4167e8ca31f8/pint/facets/measurement/objects.py#L24
[33]: https://en.wikipedia.org/wiki/Propagation_of_uncertainty
